...
namespace PluralVideos.Helpers
{
    public static class DecryptFileHelper
    {
        ...
        {
            var decryptedFolder = $@"{outputFolder}\{courseTitle.RemoveInvalidCharacter()}\{module.ModuleIndex}. {module.Title.RemoveInvalidCharacter()}";
            var fi = new FileInfo($@"{decryptedFolder}\{clip.ClipIndex}. {clip.Title.RemoveInvalidCharacter()}.mp4");
        ...
        {
            var decryptedFolder = $@"{outputFolder}\{courseTitle.RemoveInvalidCharacter()}\{module.ModuleIndex}. {module.Title.RemoveInvalidCharacter()}";
            var fi = new FileInfo($@"{decryptedFolder}\{clip.ClipIndex}. {clip.Title.RemoveInvalidCharacter()}.srt");
            if (!fi.Directory.Exists)
                fi.Directory.Create();
            return fi.Create();
        }
